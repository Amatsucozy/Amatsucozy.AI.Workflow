...
| Concept-level question in a multi-project repo ("where does payment live?") | read the `## Project Map` section of the repo's `CLAUDE.md` if it has one (see the Project Map reference below) to pick the project, then Glob/Grep for the symbol, then roslyn for positions, callers, and implementers — roslyn has no workspace-wide symbol search, so the file always comes from Glob/Grep first |
...
## Reference — Project Map (optional)

A repository with several projects may carry a `## Project Map` section in
its own `CLAUDE.md`: one row per project — `Project | Path | Owns` — where
Owns is 3–6 plain-language phrases for what the project is responsible for
(`HTTP endpoints, user auth routes, request validation`, not `API project`).
It exists for concept-level questions ("where does the payment feature
live?") that neither Grep nor roslyn can answer, because they need a symbol
name to start from. It is orientation, never evidence: every location still
comes from a Glob/Grep hit or a roslyn result. Maintained by hand — a
project with no row is simply not on the map, not absent from the repo.
