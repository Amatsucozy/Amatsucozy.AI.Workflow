...
| Experience routing concerns | `implementation` always + `tests` when the work adds/changes/fixes tests; one `exp_search` per concern; per-concern declaration in Strategy; test phases carry a `Conventions:` line | CLAUDE.md routing steps 1/3/4/5/7, skills/sdlc-orchestrator steps 2–5, skills/planning | observed 2026-09-14: routing ran once on the implementation problem statement, so test lessons were never searched even when the task wrote or fixed tests, and engineers invented test style — inconsistent tests that no gate catches | `tests`-concern searches return zero confirmed matches across 10+ test-writing tasks (the corpus has no test lessons — fix the write bar, not the routing); or test phases still drift with the lessons and Conventions attached (attachment isn't the failure — look at the engineer prompt) |
