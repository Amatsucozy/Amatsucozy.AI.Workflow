...
changed) and ticket frontmatter (where the pipeline stands). Only the final
report is written to disk, because it outlives the conversation as the PR body.
...
## Durable Progress (ticket frontmatter)

At every phase boundary and gate, update `docs/tasks/<id>/ticket.md`
frontmatter — this is what resume reads, replacing on-disk loop reports:
...
Four fields, updated in place — a few tokens per boundary, and combined with
git it reconstructs full task state at any resume.
...
and gate verdicts reconstruct most of it from git). If tuning decisions start
needing harder numbers, the escalation is resuming file-based telemetry — see
TUNING.md.
