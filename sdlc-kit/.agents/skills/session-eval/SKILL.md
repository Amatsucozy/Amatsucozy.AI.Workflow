---
name: session-eval
description: Evaluate and score an AI-user conversation when the user requests a session review, conversation rating, or retrospective. Produce weighted scores, a correction timeline, and actionable recommendations. Do not evaluate ordinary conversations without a request.
---
...
If the session is the **current conversation**, use the full conversation history visible in your context — including all prior turns available. State any missing or compacted history and limit conclusions to visible evidence. Be objective despite having participated.
...
Compute the overall score as the sum of each dimension score multiplied by its weight (weights total 100%).
...
By default, output the report as **formatted text in the chat**. If the user asks for a file, save to the requested path, or `docs/reports/session-eval-[topic].md` in the workspace, and return a clickable file link.
