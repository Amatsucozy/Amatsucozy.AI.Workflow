---
name: reporting
description: Report progress and consolidate a final report for an active SDLC task under docs/tasks. Maintain main.yaml at phase boundaries and gates; use the final report as a draft PR body.
---
...
- **Changed this turn** comes from git (`git diff --stat`), summarized to
  counts and areas. Include untracked task files from `git status --short`,
  because `git diff --stat` does not include them.
...
telemetry when explicitly requested; it is not required for this workflow.