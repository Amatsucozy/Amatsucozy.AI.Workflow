---
name: run-build
description: Compile a .NET solution or project through amtcz sarif_build and return a capped, structured error report. Use for requested builds and verification gates. Requires sarif_build and sarif_probe; reports failures without fixing code.
---
...
own initiative. Discover the configured amtcz MCP operations and inspect their input schemas
before calling them; tool prefixes vary by installation. Use AGENTS.md for
additional project guidance when present.

## Procedure

1. Target: use the named .sln/.csproj; else use `rg --files -g "*.sln"` at root (single hit
